          ...
          <div class="col-lg-12 d-flex align-items-stretch">
            <div class="card w-100">
              ...
              @if ($errors->any())
                ...
                <h5 class="card-title fw-semibold mb-4">Create New Inspection for Medical Record #{{ $medicalRecord->id }}</h5>
                
                <form action="{{ route('doctor.medical.record.inspections.store', $medicalRecord) }}" method="POST">
                    @csrf
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="datetime-local" class="form-control @error('date_time') is-invalid @enderror" 
                                   id="date_time" name="date_time" value="{{ old('date_time') }}" required>
                            @error('date_time')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <textarea class="form-control @error('result') is-invalid @enderror" 
                                      id="result" name="result" rows="3" required>{{ old('result') }}</textarea>
                            @error('result')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <textarea class="form-control @error('medicine') is-invalid @enderror" 
                                      id="medicine" name="medicine" rows="3" required>{{ old('medicine') }}</textarea>
                            @error('medicine')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-12">
                            <button type="submit" class="btn btn-primary">
                                <i class="ti ti-plus me-2"></i>Create Inspection
                            </button>
